...
def main():
    ...
    cmd_ipbb_add_mp7 = "ipbb add git {args.mp7url} -b {args.tag}_amc502_extcond".format(**locals())
    ...
    command = 'bash -c "cd; cd {ipbb_dir}/src/ugt; rm -rf mp7_ugt && rm -rf amc502_finor && rm -rf amc502_finor_pre && rm -rf mp7_tdf"'.format(**locals())
    run_command(command)
    ...
    cmd_ipbb_proj_create = "ipbb proj create vivado mp7_ugt_{build_name} mp7:../ugt/{project_type}".format(**locals())
