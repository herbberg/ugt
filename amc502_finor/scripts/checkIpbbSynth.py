...
def main():
    ...
    build_version = config.get('firmware', 'build')

    build_path = os.path.dirname(args.config)
    # IPBB 'proj' directory
    project_path = os.path.join(build_path, 'proj', '{}_{}_0x{}').format(board_type, fw_type, build_version)
    log_hr("=")
    log_info("{0} {1} synthesis results".format(board_type, fw_type))
    log_hr("=")
    log_info("")
    find_errors(project_path, 0, args)
    log_info("")
